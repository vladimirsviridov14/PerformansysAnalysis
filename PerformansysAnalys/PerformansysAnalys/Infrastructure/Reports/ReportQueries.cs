    ...
    public static class ReportQueries
    {
        ...
        WITH StudentScores AS( 
			...
			FROM students s  
				...
				JOIN courses c ON d.id = g.courseid
				JOIN projects p ON p.id = g.projectid
				LEFT JOIN attempts a ON s.id = a.studentid
			WHERE (g.directionid IS NULL OR directionid = @directionid)
			...
			SELECT 
				 ...
				 ss.Direction,
				 ss.Course,
				 (CASE WHEN ss.totalscore = max_score THEN ss.fullname END) AS LeadersStudent),
				 (CASE WHEN ss.totalscore = max_score THEN ss.totalscore END) AS LeadersStudent),
				 (CASE WHEN ss.totalscore = min_score THEN ss.totalscore END) AS LaggardsStudent),
				 (CASE WHEN ss.totalscore = min_score THEN ss.fullname END) AS LaggardsStudent),
  ...
  //      public const string StudentRating = @"
		...
		public const string StudentPassRate = @"
			...
			ROUND(
				...
				COUNT(DISTINCT tg.testsid)::NUMERIC * 100, 0
			) AS PassRate
		FROM
			students s
		...
		LEFT JOIN testresults tr ON tr.testsid = t.id AND tr.studentid = s.id AND tr.passed = TRUE
